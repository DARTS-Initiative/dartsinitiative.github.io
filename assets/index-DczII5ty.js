...
`,W=[],U=Object.freeze(Object.defineProperty({__proto__:null,attributes:N,html:X,toc:W},Symbol.toStringTag,{value:"Module"})),m={siteTitle:"DARTS",siteDescription:"Open ecosystem to research and share advances in <strong>AI that understands</strong>.",logo:{light:{svg:"/images/darts_logo.svg",png:"/images/darts_logo.png"},dark:{svg:"/images/darts_logo_black.svg",png:"/images/darts_logo_black.png"},alt:"DARTS Logo"},blog:{title:"Latest Updates"},newsletterUrl:"https://buttondown.com/api/emails/embed-subscribe/DARTS",footer:{copyright:"© 2026 DARTS Initiative - Pablo Magariños Docampo",license:"Content distributed under open licenses | Committed to Open Source & Open Access",links:[{text:"GitHub",url:"https://github.com/DARTS-Initiative"},{text:"License",url:"https://creativecommons.org/licenses/by/4.0/"},{text:"Contribute",url:"#/page/contributing"}]}};function M(e,o){document.title=e?`${e} | DARTS`:"DARTS";let t=document.querySelector('meta[name="description"]');t||(t=document.createElement("meta"),t.name="description",document.head.appendChild(t)),t.content=o||"Decentralized Autonomous Research & Technology System - Advancing open science and technology.";const r=(a,i)=>{let u=document.querySelector(`meta[property="${a}"]`)||document.querySelector(`meta[name="${a}"]`);u&&(u.content=i||"")},s=e?`${e} | DARTS`:"DARTS",n=o||"Decentralized Autonomous Research & Technology System - Advancing open science and technology.";r("og:title",s),r("og:description",n),r("twitter:title",s),r("twitter:description",n)}function A(e){e.innerHTML=`
        ...
        `}function E(e){return e?e.replace(/<img ([^>]*alt=")([^"|]*)\|([^"|]*)("[^>]*>)/g,(o,t,r,s,n)=>{const a=s.trim();return/^\d+(px|%|em|rem|vw|vh)$/.test(a)?`<img ${t}${r}" style="width: 100%; max-width: ${a};" ${n}`:o}):""}const G="modulepreload",V=function(e){return"/"+e},R={},K=function(o,t,r){let s=Promise.resolve();if(t&&t.length>0){let u=function(c){return Promise.all(c.map(h=>Promise.resolve(h).then(d=>({status:"fulfilled",value:d}),d=>({status:"rejected",reason:d}))))};document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),i=a?.nonce||a?.getAttribute("nonce");s=u(t.map(c=>{if(c=V(c),c in R)return;R[c]=!0;const h=c.endsWith(".css"),d=h?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${d}`))return;const p=document.createElement("link");if(p.rel=h?"stylesheet":G,h||(p.as="script"),p.crossOrigin="",p.href=c,i&&p.setAttribute("nonce",i),document.head.appendChild(p),h)return new Promise((w,f)=>{p.addEventListener("load",w),p.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${c}`)))})}))}function n(a){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=a,window.dispatchEvent(i),!i.defaultPrevented)throw a}return s.then(a=>{for(const i of a||[])i.status==="rejected"&&n(i.reason);return o().catch(n)})},L={News:{color:"#10b981",backgroundColor:"rgba(16, 185, 129, 0.1)",borderColor:"rgba(16, 185, 129, 0.2)"},Articles:{color:"#8b5cf6",backgroundColor:"rgba(139, 92, 246, 0.1)",borderColor:"rgba(139, 92, 246, 0.2)"},Others:{color:"#6b7280",backgroundColor:"rgba(107, 114, 128, 0.1)",borderColor:"rgba(107, 114, 128, 0.2)"},default:{color:"#6b7280",backgroundColor:"rgba(107, 114, 128, 0.1)",borderColor:"rgba(107, 114, 128, 0.2)"}},J=Object.assign({"../../content/posts/welcome-to-darts.md":()=>K(()=>import("./welcome-to-darts-VqW7cEBo.js"),[])});let k=[];async function Q(){try{const e=await fetch("/posts-index.json");if(!e.ok)throw new Error("Failed to load posts index");k=(await e.json()).posts||[]}catch(e){console.error("Error loading posts index:",e),k=[]}}function Z(){return k}function ee(e){M("Blog","Read the latest news and updates from the DARTS Initiative.");const o=k.map(t=>{const r=t.slug;return`
            <a href="#/post/${r}" class="post-card" data-slug="${r}">
                <h3>${t.title}</h3>
                <div class="post-meta" style="${_(t.category)}">
    ...
    `}async function te(e,o){const t=`../../content/posts/${o}.md`,r=J[t];if(!r){A(e);return}try{const s=await r(),{attributes:n,html:a}=s;M(n.title,n.excerpt||n.description),e.innerHTML=`
            <div class="post-detail">
                <a href="#/blog" class="back-button">Back to Blog</a>
                <h1>${n.title}</h1>
                <div class="post-meta" style="${_(n.category)}">
            ...
            `}catch(s){console.error("Error loading post:",s),A(e)}}function _(e){const o=L[e]||L.default;return`
        color: ${o.color};
...
`,he=[],pe=Object.freeze(Object.defineProperty({__proto__:null,attributes:de,html:ue,toc:he},Symbol.toStringTag,{value:"Module"})),C=Object.assign({"../../content/pages/about-us.md":re,"../../content/pages/contributing.md":ce,"../../content/pages/roadmap.md":pe});function ge(){return C}function me(e,o){const t=`../../content/pages/${o}.md`,r=C[t];if(!r){A(e);return}M(r.attributes.title,r.attributes.excerpt||r.attributes.description),e.innerHTML=`
  ...
  `,document.body.appendChild(e),requestAnimationFrame(()=>{requestAnimationFrame(()=>{e.classList.add("newsletter-visible")})});const o=()=>{T({dismissedAt:Date.now()}),x(e),document.removeEventListener("keydown",t)};e.querySelector(".newsletter-close").addEventListener("click",o),e.querySelector(".newsletter-form").addEventListener("submit",()=>{T({subscribedAt:Date.now()}),document.removeEventListener("keydown",t),setTimeout(()=>x(e),300)}),e.querySelector(".newsletter-already").addEventListener("click",()=>{T({subscribedAt:Date.now()}),document.removeEventListener("keydown",t),x(e)});const t=r=>{r.key==="Escape"&&o()};document.addEventListener("keydown",t)}function ye(){T({subscribedAt:Date.now()})}function Ae(){document.querySelector(".newsletter-banner")||F()}function Se(){we()&&setTimeout(F,5e3)}const $e=Object.assign({"../../content/intro.md":U});function Me(e){M(null,m.siteDescription);const o=ge(),t=Z(),r=Object.entries(o).map(([i,u])=>`
    ...
    `).join(""),s=t.slice(0,2).map(i=>{const u=i.slug;return`
            <a href="#/post/${u}" class="post-card" data-slug="${u}">
                <h3>${i.title}</h3>
                <div class="post-meta" style="${_(i.category)}">
        ...
        `}).join(""),n=E($e["../../content/intro.md"]?.html);e.innerHTML=`
    <div class="logo-wrapper">
        ...
        `,e.querySelectorAll(".logo-light").forEach(i=>{i.addEventListener("error",function(){this.src=m.logo.light.png},{once:!0})}),e.querySelectorAll(".logo-dark").forEach(i=>{i.addEventListener("error",function(){this.src=m.logo.dark.png},{once:!0})});{const i=e.querySelectorAll(".logo");let u=1,c=0,h=null;const d=()=>{if(c=c*.872+(1-u)*.08,u+=c,Math.abs(1-u)<5e-4&&Math.abs(c)<5e-4){u=1,c=0,i.forEach(S=>{S.style.transform=""}),h=null;return}i.forEach(S=>{S.style.transform=`scale(${u})`}),h=requestAnimationFrame(d)};let p=0,w=!1;i.forEach(f=>{f.style.cursor="pointer",f.style.willChange="transform",f.addEventListener("click",()=>{if(!w&&(u=Math.max(.3,u-.22),c=0,h&&cancelAnimationFrame(h),h=requestAnimationFrame(d),p++,p===10)){w=!0,h&&cancelAnimationFrame(h);const b=e.querySelector(".logo-wrapper");b&&(b.dispatchEvent(new Event("escaped")),b.style.transition="transform 0.6s cubic-bezier(0.4, 0, 1, 1), opacity 0.6s ease",b.style.transform="translateX(150%)",b.style.opacity="0")}})})}if(!window.matchMedia("(prefers-reduced-motion: reduce)").matches){const i=e.querySelector(".logo-wrapper");if(i){const u=i.getBoundingClientRect().top;let c=!1;i.addEventListener("escaped",()=>{c=!0});const h=()=>{if(!i.isConnected){window.removeEventListener("scroll",h);return}if(c)return;const d=i.getBoundingClientRect(),p=d.height;if(!p)return;const w=u+p,f=Math.max(0,Math.min(1,(u-d.top)/w)),b=2*f*f*f;i.style.transform=b>0?`translateX(${b*130}%)`:"",i.style.opacity=b>0?1-f:""};window.addEventListener("scroll",h,{passive:!0})}}const a=e.querySelector(".subscribe-form");a&&a.addEventListener("submit",()=>ye())}const Te=Object.assign({});let D=[];async function ke(){try{const e=await fetch("/projects-index.json");if(!e.ok)throw new Error("Failed to load projects index");D=(await e.json()).projects||[]}catch(e){console.error("Error loading projects index:",e),D=[]}}function Ee(e){M("Projects","Discover the innovative projects developed by the DARTS Initiative.");const o=D.map(t=>{const r=t.slug;return`
        ...
        `}async function Ie(e,o){const t=`../../content/projects/${o}.md`,r=Te[t];if(!r){A(e);return}try{const s=await r(),{attributes:n,html:a}=s;M(n.title,n.excerpt||n.description),e.innerHTML=`
            <div class="post-detail">
            ...
            `}catch(s){console.error("Error loading project:",s),A(e)}}function P(e){return/^[a-z0-9-]+$/i.test(e)}function je(e){const o=document.getElementById(e);function t(s){document.querySelectorAll(".nav-link").forEach(n=>{n.classList.remove("active"),n.getAttribute("href")===s&&n.classList.add("active")}),(s===""||s==="#/")&&document.querySelector('.nav-link[href="#/"]')?.classList.add("active")}async function r(){const s=window.location.hash;try{if(window.scrollTo(0,0),t(s),s===""||s==="#/")Me(o);else if(s==="#/blog")ee(o);else if(s==="#/projects")Ee(o);else if(s.startsWith("#/post/")){const n=s.replace("#/post/","");P(n)?await te(o,n):A(o)}else if(s.startsWith("#/project/")){const n=s.replace("#/project/","");P(n)?await Ie(o,n):A(o)}else if(s.startsWith("#/page/")){const n=s.replace("#/page/","");P(n)?me(o,n):A(o)}else A(o)}catch(n){console.error("Route error:",n),A(o)}}r(),window.addEventListener("hashchange",r)}function xe(){let e=document.querySelector("footer");e||(e=document.createElement("footer"),document.body.appendChild(e));const o=m.footer.links.map(t=>`<a href="${t.url}" target="${t.url.startsWith("http")?"_blank":"_self"}" ${t.url.startsWith("http")?'rel="noopener noreferrer"':""}>${t.text}</a>`).join("");e.innerHTML=`
    ...
    `,e.querySelector(".footer-subscribe").addEventListener("click",t=>{t.preventDefault(),Ae()})}function Pe(){const e=localStorage.getItem("theme"),o=window.matchMedia("(prefers-color-scheme: dark)").matches;e==="dark"||!e&&o?document.documentElement.setAttribute("data-theme","dark"):document.documentElement.setAttribute("data-theme","light")}function De(){const o=document.documentElement.getAttribute("data-theme")==="dark"?"light":"dark";return document.documentElement.setAttribute("data-theme",o),localStorage.setItem("theme",o),o}function _e(){const e=document.getElementById("theme-toggle");e&&e.addEventListener("click",()=>{De()})}Pe();_e();z("background-canvas");try{await Q()}catch(e){console.error("Failed to load posts:",e)}try{await ke()}catch(e){console.error("Failed to load projects:",e)}je("app");xe();Se();
