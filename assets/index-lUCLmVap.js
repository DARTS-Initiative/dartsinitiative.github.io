...
`,W=[],G=Object.freeze(Object.defineProperty({__proto__:null,attributes:z,html:X,toc:W},Symbol.toStringTag,{value:"Module"})),u={siteTitle:"DARTS",siteDescription:"Open ecosystem to research and share advances in <strong>AI that understands</strong>.",logo:{light:{svg:"/images/darts_logo.svg",png:"/images/darts_logo.png"},dark:{svg:"/images/darts_logo_black.svg",png:"/images/darts_logo_black.png"},alt:"DARTS Logo"},blog:{title:"Latest Updates"},newsletterUrl:"https://buttondown.com/api/emails/embed-subscribe/DARTS",footer:{copyright:"© 2026 DARTS Initiative - Pablo Magariños Docampo",license:"Content distributed under open licenses | Committed to Open Source & Open Access",links:[{text:"GitHub",url:"https://github.com/DARTS-Initiative"},{text:"License",url:"#license"},{text:"Contribute",url:"#contribute"}]}};function y(e,o){document.title=e?`${e} | DARTS`:"DARTS";let t=document.querySelector('meta[name="description"]');t||(t=document.createElement("meta"),t.name="description",document.head.appendChild(t)),t.content=o||"Decentralized Autonomous Research & Technology System - Advancing open science and technology.";const r=(a,i)=>{let m=document.querySelector(`meta[property="${a}"]`)||document.querySelector(`meta[name="${a}"]`);m&&(m.content=i||"")},s=e?`${e} | DARTS`:"DARTS",n=o||"Decentralized Autonomous Research & Technology System - Advancing open science and technology.";r("og:title",s),r("og:description",n),r("twitter:title",s),r("twitter:description",n)}function v(e){e.innerHTML=`
        ...
        `}function T(e){return e?e.replace(/<img ([^>]*alt=")([^"|]*)\|([^"|]*)("[^>]*>)/g,(o,t,r,s,n)=>{const a=s.trim();return/^\d+(px|%|em|rem|vw|vh)$/.test(a)?`<img ${t}${r}" style="width: 100%; max-width: ${a};" ${n}`:o}):""}const U="modulepreload",K=function(e){return"/"+e},R={},V=function(o,t,r){let s=Promise.resolve();if(t&&t.length>0){let m=function(h){return Promise.all(h.map(f=>Promise.resolve(f).then(d=>({status:"fulfilled",value:d}),d=>({status:"rejected",reason:d}))))};document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),i=a?.nonce||a?.getAttribute("nonce");s=m(t.map(h=>{if(h=K(h),h in R)return;R[h]=!0;const f=h.endsWith(".css"),d=f?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${h}"]${d}`))return;const p=document.createElement("link");if(p.rel=f?"stylesheet":U,f||(p.as="script"),p.crossOrigin="",p.href=h,i&&p.setAttribute("nonce",i),document.head.appendChild(p),f)return new Promise((A,k)=>{p.addEventListener("load",A),p.addEventListener("error",()=>k(new Error(`Unable to preload CSS for ${h}`)))})}))}function n(a){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=a,window.dispatchEvent(i),!i.defaultPrevented)throw a}return s.then(a=>{for(const i of a||[])i.status==="rejected"&&n(i.reason);return o().catch(n)})},L={News:{color:"#10b981",backgroundColor:"rgba(16, 185, 129, 0.1)",borderColor:"rgba(16, 185, 129, 0.2)"},Articles:{color:"#8b5cf6",backgroundColor:"rgba(139, 92, 246, 0.1)",borderColor:"rgba(139, 92, 246, 0.2)"},Others:{color:"#6b7280",backgroundColor:"rgba(107, 114, 128, 0.1)",borderColor:"rgba(107, 114, 128, 0.2)"},default:{color:"#6b7280",backgroundColor:"rgba(107, 114, 128, 0.1)",borderColor:"rgba(107, 114, 128, 0.2)"}},J=Object.assign({"../../content/posts/demo-post.md":()=>V(()=>import("./demo-post-VqW7cEBo.js"),[])});let $=[];async function Q(){try{const e=await fetch("/posts-index.json");if(!e.ok)throw new Error("Failed to load posts index");$=(await e.json()).posts||[]}catch(e){console.error("Error loading posts index:",e),$=[]}}function Z(){return $}function ee(e){y("Blog","Read the latest news and updates from the DARTS Initiative.");const o=$.map(t=>{const r=t.slug;return`
    ...
    `}async function te(e,o){const t=`../../content/posts/${o}.md`,r=J[t];if(!r){v(e);return}try{const s=await r(),{attributes:n,html:a}=s;y(n.title,n.excerpt||n.description),e.innerHTML=`
...
`,he=[],pe=Object.freeze(Object.defineProperty({__proto__:null,attributes:de,html:ue,toc:he},Symbol.toStringTag,{value:"Module"})),O=Object.assign({"../../content/pages/about-us.md":re,"../../content/pages/contributing.md":ce,"../../content/pages/roadmap.md":pe});function ge(){return O}function me(e,o){const t=`../../content/pages/${o}.md`,r=O[t];if(!r){v(e);return}y(r.attributes.title,r.attributes.excerpt||r.attributes.description),e.innerHTML=`
  ...
  `,document.body.appendChild(e),requestAnimationFrame(()=>{requestAnimationFrame(()=>{e.classList.add("newsletter-visible")})});const o=()=>{S({dismissedAt:Date.now()}),j(e),document.removeEventListener("keydown",t)};e.querySelector(".newsletter-close").addEventListener("click",o),e.querySelector(".newsletter-form").addEventListener("submit",()=>{S({subscribedAt:Date.now()}),setTimeout(()=>j(e),300)}),e.querySelector(".newsletter-already").addEventListener("click",()=>{S({subscribedAt:Date.now()}),j(e)});const t=r=>{r.key==="Escape"&&o()};document.addEventListener("keydown",t)}function ye(){S({subscribedAt:Date.now()})}function Ae(){document.querySelector(".newsletter-banner")||F()}function Se(){we()&&setTimeout(F,5e3)}const $e=Object.assign({"../../content/intro.md":G});function Te(e){y(null,u.siteDescription);const o=ge(),t=Z(),r=Object.entries(o).map(([i,m])=>`
        <a href="#/page/${i.split("/").pop().replace(".md","")}" class="section-box">
            <h3>${m.attributes.title}</h3>
        </a>
    `).join(""),s=t.slice(0,2).map(i=>{const m=i.slug;return`
    ...
    <img 
        ...
        <div class="info-text">
            <p>
                ...
                <a href="${u.footer.links[0].url}" target="_blank" rel="noopener noreferrer">GitHub repository</a>.
            </p>
        </div>
        `,e.querySelectorAll(".logo-light").forEach(i=>{i.addEventListener("error",function(){this.src=u.logo.light.png},{once:!0})}),e.querySelectorAll(".logo-dark").forEach(i=>{i.addEventListener("error",function(){this.src=u.logo.dark.png},{once:!0})});const a=e.querySelector(".subscribe-form");a&&a.addEventListener("submit",()=>ye())}const ke=Object.assign({});let E=[];async function Ie(){try{const e=await fetch("/projects-index.json");if(!e.ok)throw new Error("Failed to load projects index");E=(await e.json()).projects||[]}catch(e){console.error("Error loading projects index:",e),E=[]}}function Me(e){y("Projects","Discover the innovative projects developed by the DARTS Initiative.");const o=E.map(t=>{const r=t.slug;return`
        ...
        `}async function je(e,o){const t=`../../content/projects/${o}.md`,r=ke[t];if(!r){v(e);return}try{const s=await r(),{attributes:n,html:a}=s;y(n.title,n.excerpt||n.description),e.innerHTML=`
            <div class="post-detail">
            ...
            `}catch(s){console.error("Error loading project:",s),v(e)}}function x(e){return/^[a-z0-9-]+$/i.test(e)}function xe(e){const o=document.getElementById(e);function t(s){document.querySelectorAll(".nav-link").forEach(n=>{n.classList.remove("active"),n.getAttribute("href")===s&&n.classList.add("active")}),(s===""||s==="#/")&&document.querySelector('.nav-link[href="#/"]')?.classList.add("active")}async function r(){const s=window.location.hash;if(window.scrollTo(0,0),t(s),s===""||s==="#/")Te(o);else if(s==="#/blog")ee(o);else if(s==="#/projects")Me(o);else if(s.startsWith("#/post/")){const n=s.replace("#/post/","");x(n)?await te(o,n):v(o)}else if(s.startsWith("#/project/")){const n=s.replace("#/project/","");x(n)?await je(o,n):v(o)}else if(s.startsWith("#/page/")){const n=s.replace("#/page/","");x(n)?me(o,n):v(o)}else v(o)}r(),window.addEventListener("hashchange",r)}function Ee(){let e=document.querySelector("footer");e||(e=document.createElement("footer"),document.body.appendChild(e));const o=u.footer.links.map(t=>`<a href="${t.url}" target="${t.url.startsWith("http")?"_blank":"_self"}" ${t.url.startsWith("http")?'rel="noopener noreferrer"':""}>${t.text}</a>`).join("");e.innerHTML=`
    ...
    `,e.querySelector(".footer-subscribe").addEventListener("click",t=>{t.preventDefault(),Ae()})}function Pe(){const e=localStorage.getItem("theme"),o=window.matchMedia("(prefers-color-scheme: dark)").matches;e==="dark"||!e&&o?document.documentElement.setAttribute("data-theme","dark"):document.documentElement.setAttribute("data-theme","light")}function _e(){const o=document.documentElement.getAttribute("data-theme")==="dark"?"light":"dark";return document.documentElement.setAttribute("data-theme",o),localStorage.setItem("theme",o),o}function De(){const e=document.getElementById("theme-toggle");e&&e.addEventListener("click",()=>{_e()})}Pe();De();N("background-canvas");await Q();await Ie();xe("app");Ee();Se();
