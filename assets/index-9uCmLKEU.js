(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))i(e);new MutationObserver(e=>{for(const o of e)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function n(e){const o={};return e.integrity&&(o.integrity=e.integrity),e.referrerPolicy&&(o.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?o.credentials="include":e.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(e){if(e.ep)return;e.ep=!0;const o=n(e);fetch(e.href,o)}})();function C(t){const s=document.getElementById(t);if(!s)return;const n=s.getContext("2d");let i=s.width=window.innerWidth,e=s.height=window.innerHeight,o=-1e3,r=-1e3;const u=/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)||"ontouchstart"in window||navigator.maxTouchPoints>0;window.addEventListener("resize",()=>{i=s.width=window.innerWidth,e=s.height=window.innerHeight,j()}),u||(window.addEventListener("mousemove",d=>{o=d.clientX,r=d.clientY}),window.addEventListener("mouseleave",()=>{o=-1e3,r=-1e3})),new MutationObserver(()=>{c.baseColor=S()}).observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]});function $(d){const a=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(d);return a?{r:parseInt(a[1],16),g:parseInt(a[2],16),b:parseInt(a[3],16)}:null}function S(){return document.documentElement.getAttribute("data-theme")==="dark"?{...$("#FFFFFF"),a:.018}:{...$("#000000"),a:.07}}const c={size:40,speed:.18,baseColor:S(),hoverColor:{...$("#C2A365"),a:.65},rows:16,hoverRadius:90,transitionSpeed:.1,mobileGlowChance:3e-4,mobileGlowFadeInSpeed:.04,mobileGlowFadeOutSpeed:.015};let w=[];class P{constructor(a,h){this.x=a,this.y=h,this.size=c.size,this.speed=c.speed+(Math.random()-.5)*.2,this.hoverAmount=0,this.isGlowing=!1,this.glowPhase="idle"}isMouseNear(){const a=this.x+this.size/2,h=this.y+this.size/2;return Math.sqrt((o-a)**2+(r-h)**2)<c.hoverRadius}draw(){const a=c.baseColor,h=c.hoverColor,g=this.hoverAmount,A=Math.round(a.r+(h.r-a.r)*g),T=Math.round(a.g+(h.g-a.g)*g),O=Math.round(a.b+(h.b-a.b)*g),D=a.a+(h.a-a.a)*g;n.beginPath(),n.moveTo(this.x,this.y),n.lineTo(this.x+this.size,this.y+this.size/2),n.lineTo(this.x,this.y+this.size),n.closePath(),n.fillStyle=`rgba(${A}, ${T}, ${O}, ${D})`,n.fill()}update(){this.x+=this.speed,this.x>i+this.size&&(this.x=-this.size*2),u?(this.glowPhase==="idle"&&Math.random()<c.mobileGlowChance&&(this.glowPhase="fadeIn"),this.glowPhase==="fadeIn"?(this.hoverAmount=Math.min(1,this.hoverAmount+c.mobileGlowFadeInSpeed),this.hoverAmount>=1&&(this.glowPhase="fadeOut")):this.glowPhase==="fadeOut"&&(this.hoverAmount=Math.max(0,this.hoverAmount-c.mobileGlowFadeOutSpeed),this.hoverAmount<=0&&(this.glowPhase="idle"))):this.isMouseNear()?this.hoverAmount=Math.min(1,this.hoverAmount+c.transitionSpeed):this.hoverAmount=Math.max(0,this.hoverAmount-c.transitionSpeed)}}function j(){w=[];const d=e/c.rows;for(let a=0;a<c.rows;a++){const h=Math.ceil(i/(c.size*2));for(let g=0;g<h+2;g++){const A=g*(c.size*2)-c.size*3+(a%2===0?0:c.size),T=a*d+Math.random()*d/4;w.push(new P(A,T))}}}function k(){n.clearRect(0,0,i,e),w.forEach(d=>{d.update(),d.draw()}),requestAnimationFrame(k)}j(),k()}const R={title:"Introduction"},I=`<p>DARTS (Drone Autonomous Research and Technology for Swarms) is a cutting-edge initiative designed to democratize access to advanced swarm technology.</p>
<h3>The Mission</h3>
...
`,E=[{level:"3",content:"The Mission"},{level:"3",content:"Future Work"}],L=Object.freeze(Object.defineProperty({__proto__:null,attributes:R,html:I,toc:E},Symbol.toStringTag,{value:"Module"})),l={siteTitle:"DARTS",siteDescription:"Open ecosystem to research and share advances in <strong>AI that understands</strong>.",logo:{light:{svg:"/images/darts_logo.svg",png:"/images/darts_logo.png"},dark:{svg:"/images/darts_logo_black.svg",png:"/images/darts_logo_black.png"},alt:"DARTS Logo"},blog:{title:"Latest Updates"},footer:{copyright:"© 2026 DARTS Initiative - Pablo Magariños Docampo",license:"Content distributed under open licenses | Committed to Open Source & Open Access",links:[{text:"GitHub",url:"https://github.com/DARTS-Initiative"},{text:"License",url:"#license"},{text:"Contribute",url:"#contribute"}]}};function m(t,s){document.title=t?`${t} | DARTS`:"DARTS";let n=document.querySelector('meta[name="description"]');n||(n=document.createElement("meta"),n.name="description",document.head.appendChild(n)),n.content=s||"Decentralized Autonomous Research & Technology System - Advancing open science and technology.";const i=(r,u)=>{let b=document.querySelector(`meta[property="${r}"]`)||document.querySelector(`meta[name="${r}"]`);b&&(b.content=u||"")},e=t?`${t} | DARTS`:"DARTS",o=s||"Decentralized Autonomous Research & Technology System - Advancing open science and technology.";i("og:title",e),i("og:description",o),i("twitter:title",e),i("twitter:description",o)}function p(t){t.innerHTML=`
...
`,Z=[{level:"1",content:"DARTS Manifesto"}],tt=Object.freeze(Object.defineProperty({__proto__:null,attributes:J,html:Q,toc:Z},Symbol.toStringTag,{value:"Module"})),M=Object.assign({"../../content/pages/about-us.md":X,"../../content/pages/contributing.md":V,"../../content/pages/manifesto.md":tt});function et(){return M}function ot(t,s){const n=`../../content/pages/${s}.md`,i=M[n];if(!i){p(t);return}m(i.attributes.title,i.attributes.excerpt||i.attributes.description),t.innerHTML=`
        ...
        `}const st=Object.assign({"../../content/intro.md":L});function nt(t){m(null,l.siteDescription);const s=et(),n=F(),i=Object.entries(s).map(([r,u])=>`
        <a href="#/page/${r.split("/").pop().replace(".md","")}" class="section-box">
            <h3>${u.attributes.title}</h3>
        </a>
    `).join(""),e=n.slice(0,2).map(r=>{const u=r.slug;return`
    ...
    `}function ht(){const t=localStorage.getItem("theme"),s=window.matchMedia("(prefers-color-scheme: dark)").matches;t==="dark"||!t&&s?document.documentElement.setAttribute("data-theme","dark"):document.documentElement.setAttribute("data-theme","light")}function ut(){const s=document.documentElement.getAttribute("data-theme")==="dark"?"light":"dark";return document.documentElement.setAttribute("data-theme",s),localStorage.setItem("theme",s),s}function gt(){const t=document.getElementById("theme-toggle");t&&t.addEventListener("click",()=>{ut()})}ht();gt();C("background-canvas");await H();await rt();lt("app");dt();console.log("DARTS Initiative Website Initialized");
